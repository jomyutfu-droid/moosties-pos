...
begin
  ...
  -- Fail after the first stock write: the transaction must undo every earlier write.
  v_rejected := false;
  begin
    perform public.submit_line_man_order(v_token,v_sale || jsonb_build_object(
      'client_uuid',gen_random_uuid(),'reference','QA-ATOMIC-FAIL',
      'stock_movements',(v_sale->'stock_movements') || jsonb_build_array(jsonb_build_object(
        'ingredient_id',gen_random_uuid(),'type','sale','qty_delta',-1))));
  exception when foreign_key_violation then v_rejected := true;
  end;
  if not v_rejected then raise exception 'missing ingredient accepted'; end if;
  if exists(select 1 from public.orders where external_order_ref='QA-ATOMIC-FAIL' and branch_id=v_branch) then raise exception 'failed sale left an order'; end if;
  perform set_config('request.jwt.claim.sub','',true);
  v_rejected := false;
  begin
    perform public.submit_line_man_order(v_token,v_sale);
  exception when raise_exception then v_rejected := true;
  end;
  if not v_rejected then raise exception 'missing auth accepted'; end if;
  if (select stock_qty from public.ingredients where id = v_ing) <> 80 then raise exception 'rejection changed stock'; end if;
  reset role;
end;
$$;
rollback;
select 'PASS: authenticated sale, retry, duplicate ref, stock once, exact totals, invalid auth/session/ref/price, atomic rollback; all fixtures rolled back' as result;
