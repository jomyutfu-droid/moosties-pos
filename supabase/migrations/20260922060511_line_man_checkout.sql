-- Additive migration: existing store/Grab prices, orders and submit_order stay unchanged.
alter table public.products add column line_man_price numeric(12,2)
  check (line_man_price >= 0 and line_man_price <> 'NaN'::numeric);
alter table public.product_options add column line_man_price numeric(12,2)
  check (line_man_price >= 0 and line_man_price <> 'NaN'::numeric);
alter table public.orders add column checkout_source text;
alter table public.orders add column external_order_ref text;
alter table public.orders add column receipt_snapshot jsonb;
alter table public.orders add constraint orders_line_man_reference_check check (
  checkout_source is distinct from 'line_man' or
  (external_order_ref is not null and external_order_ref ~ '^[A-Z0-9][A-Z0-9_-]{0,79}$')
);
-- Keep voided references reserved too: reprinting or cancelling must not create a new sale.
create unique index orders_line_man_reference_unique
  on public.orders (coalesce(branch_id, '00000000-0000-0000-0000-000000000000'::uuid), external_order_ref)
  where checkout_source = 'line_man';
create index orders_line_man_history_idx on public.orders(branch_id, created_at desc)
  where checkout_source = 'line_man';

create function public.submit_line_man_order(p_token text, p_sale jsonb)
...
begin
  ...
  select branch_id, role into v_branch, v_role from public.users where id = v_user and is_active;
