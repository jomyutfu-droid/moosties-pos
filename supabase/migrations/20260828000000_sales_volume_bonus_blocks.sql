-- โบนัสยอดขายแบบขั้นบันได: ทุก 25 แก้ว ได้ 50 บาท
-- เช่น 25 แก้ว = 50 บาท, 50–74 แก้ว = 100 บาท
begin;
...
begin
  ...
  v_bonus_blocks := floor(p_cups_sold / 25.0)::integer;
  v_reward_amount := v_bonus_blocks * 50;
  ...
  insert into public.staff_rewards (
    ...
    v_branch_id, v_user_id, v_reward_date, 'sales_volume', p_cups_sold, v_reward_amount, 'pending',
    jsonb_build_object(
      'cups_sold', p_cups_sold,
      'bonus_blocks', v_bonus_blocks,
      'bonus_rate', 50
    ),
    v_user_id, now()
  )
