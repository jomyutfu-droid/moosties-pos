...
export function buildLineManPrintHTML(order: LineManReceiptInfo, header: string, footer: string): string {
  ...
  const summary = order.items.map(item => `<tr><td>${esc(item.productName)}${item.options ? `<br><small>${esc(item.options)}</small>` : ''}</td><td class="r">×${item.qty}</td></tr>`).join('')
